...
const FABRICS = ["Cotton", "Linen", "Wool", "Wool Blend", "Egyptian Cotton", "Oxford Cotton"];
...
export function ProductDetailClient({ product }: ProductDetailClientProps) {
  ...
  const [selectedFabric, setSelectedFabric] = React.useState(product.fabric);
  const [selectedFit, setSelectedFit] = React.useState(product.fitStyle);
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <Select value={selectedFabric} onValueChange={setSelectedFabric}>
          ...
          <SelectContent>
            {[product.fabric, ...FABRICS.filter((f) => f !== product.fabric)].map((f) => (
